...
app.delete("/todos/:todoId", (req, res) => {
  ...
  try {
    ...
    const todoIndex = user.todos.findIndex((todo) => todo.id === Number(todoId));
...
app.get("/statistic", (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ statistic: user.data.statistic });
  } catch (error) {
    return res.status(401).json({ message: "Неверный или истекший токен" });
  }
});
...
app.post("/statistic", (req, res) => {
  ...
  try {
    ...
    user.data.statistic = statistic;
    writeDatabase(db);

    return res.status(201).json({ message: "Статистика обновлена" });
  } catch (error) {
    return res.status(401).json({ message: "Неверный или истекший токен" });
  }
})
